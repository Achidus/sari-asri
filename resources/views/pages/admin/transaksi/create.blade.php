...
@push('scripts')
    ...
    // Format berat agar tidak menampilkan nol di belakang koma
    let formattedBerat = parseFloat(berat.toString());


    // Optional: jika kamu juga ingin memperlihatkan berat yang dibersihkan (bisa diinput update juga)
    row.find('input[name*="berat_kg"]').val(formattedBerat);

    let totalHarga = hargaPerKg * berat;

    // Format total harga dengan pemisah ribuan tapi tanpa koma desimal
    row.find('.total-harga').text(Math.round(totalHarga).toLocaleString());
