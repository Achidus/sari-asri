...
@section('main')
    ...
    <option value="{{ $sampah->id }}" data-stok="{{ $sampah->stok }}" data-harga="{{ $sampah->harga_per_kg }}">
        {{ $sampah->nama_sampah }}
    </option>
@endforeach
...
@push('scripts')
    ...
    if (isDuplicate) {
        ...
        $(this).closest('tr').find('.harga-input').val('');
        return;
    }

    // Set stok tersedia
    let stokTersedia = selected.data('stok') || 0;
    $(this).closest('tr').find('.stok-tersedia').text(`${stokTersedia} kg`);

    // Set harga per kg
    let hargaPerKg = selected.data('harga') || 0;
    $(this).closest('tr').find('.harga-input').val(hargaPerKg);
    ...
    // Tampilkan format ribuan ke user
    row.find('.harga-total').text(hargaTotal.toLocaleString('id-ID'));

    // Simpan nilai asli untuk perhitungan total (hindari text yang sudah diformat)
